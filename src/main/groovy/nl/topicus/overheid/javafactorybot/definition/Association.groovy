 ...
 */
class Association<T> extends BaseAssociation<T> {
     ...
     * @param traits List of traits to apply to the associated object.
     */
    Association(Class<BaseFactory<T, ? extends Faker>> factoryClass, Map<String, Object> defaultOverrides = null, List<String> traits = null) {
        super(factoryClass)
        this.defaultOverrides = defaultOverrides
        this.traits = traits
    }
     ...
     * @param defaultObject The default object to be used when no overrides are given.
     */
    Association(Class<BaseFactory<T, ? extends Faker>> factoryClass, T defaultObject) {
        super(factoryClass)
        this.withDefaultObject = true
        this.defaultObject = defaultObject
    }
    ...
    def evaluate(Evaluator evaluator) {
        if (defaultOverrides != null) {
            ...
            getFactory().build(defaultObject)
        } else {
            // Default build
            getFactory().build()
        }
